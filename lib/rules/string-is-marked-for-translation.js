...
module.exports = function(context) {
    ...
    function getFullFunctionCall(functionCallee, previous) {
        if (!previous) {
            previous = functionCallee.name || functionCallee.property.name
        }
        if (functionCallee.object) {
            if (functionCallee.name) {
                return functionCallee.name + '.' + previous
            } else if (functionCallee.object.object) {
                previous = functionCallee.object.property.name + '.' + previous
                return getFullFunctionCall(functionCallee.object.object, previous)
            }
        }
        return previous
    }
