...
module.exports = function(context) {
  ...
  function nodeIsIgnored (node) {
    ...
    const openingElement = node.openingElement || node.parent && node.parent.openingElement
    return (
      (
        openingElement &&
        ignoreTags.indexOf(openingElement.name.name) !== -1
      )
    )
  }
  ...
  // Returns the number of sibillings relative to parent
  function findIndexToParent (parent, node, index = 0) {
    if (!node) {
    return -1
    }
    if (parent === node) {
    ...
    return findIndexToParent(parent, node.parent, index + 1)
  }
  ...
  return {
    ...
    Literal: function(node) {
      const d = what => (console.log('debug:', what) || true)
      ...
      if (
        ...
        (
          ...
          (
            ...
            (
            !parentJSXIgnored ||
            // The content of the attribute is in a JSX ignored
            findIndexToParent(node.parent, parentJSXIgnored) < 0
            ) &&
