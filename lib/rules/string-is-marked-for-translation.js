            ...
            if (parentArrowFunctionName) {
                ...
                isInIgnoredFunction = ignoreFunctions.some(funcName => {
                    if (functionCallee.name) {
                        ...
                        let name = funcName
                        let obj
                        // case of object.function
                        if (funcName.indexOf('.') !== -1) {
                            [obj, name] = funcName.split('.')
                        }
                        result = name === functionCallee.property.name
                        if (obj && functionCallee.object) {
                            result = obj === functionCallee.object.name
                        }
                        return result
                    }
                })
